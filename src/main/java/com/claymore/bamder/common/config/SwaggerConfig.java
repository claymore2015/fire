...
public class SwaggerConfig {
    ...
    public Docket portalApi() {
        return new Docket(DocumentationType.SWAGGER_2).groupName("bamder-api").apiInfo(apiInfo())
                .select().apis(RequestHandlerSelectors.basePackage("com.claymore.bamder")).paths(PathSelectors.any()).build();
        // .securitySchemes(newArrayList(oauth()))
        // .securityContexts(newArrayList(securityContext()));
    }

    @SuppressWarnings("unchecked")
    private Predicate<String> bamderPortalPaths() {
        return Predicates.or(
                PathSelectors.regex("/gj.*"));
    }

    private ApiInfo apiInfo() {
        return new ApiInfoBuilder().title("Bamder portal API")
                .description(
                        "light rapid bamder project")
